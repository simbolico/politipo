...
with open("simple_test_results.txt", "w") as f:
    ...
    try:
        ...
        schema = pa.DataFrameSchema(
            {
                "name": pa.Column(
                    dtype="object", checks=[pa.Check.str_length(min_value=1, max_value=50)]
                ),
                "age": pa.Column(
                    dtype="int64", checks=[pa.Check.in_range(min_value=0, max_value=150)]
                ),
        ...
        # Test data
        df = pd.DataFrame({"name": ["Alice Smith"], "age": [30], "email": ["alice@example.com"]})
