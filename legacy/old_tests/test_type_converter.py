...
def test_pandera_schema_validation():
    ...
    schema = pa.DataFrameSchema(
        {"id": pa.Column(pa.Int, required=True), "name": pa.Column(pa.String, required=True)}
    )

    # Create a valid DataFrame
    valid_df = pd.DataFrame({"id": [1, 2, 3], "name": ["Alice", "Bob", "Charlie"]})

    # Create an invalid DataFrame (wrong type)
    invalid_df = pd.DataFrame(
        {"id": ["1", "2", "3"], "name": ["Alice", "Bob", "Charlie"]}  # Strings instead of integers
    )
...
def test_polars_with_pandera():
    ...
    valid_df = pl.DataFrame({"id": [1, 2, 3], "name": ["Alice", "Bob", "Charlie"]})
...
class TestAdvancedPydanticTypeConverter:
    ...
    def setup_method(self):
        ...
        # Sample data
        self.address_dict = {"street": "123 Main St", "city": "Anytown", "zip_code": "12345"}
        ...
        self.blog_dict = {
            ...
            "comments": [{"id": 1, "text": "Great post!"}, {"id": 2, "text": "Thanks for sharing"}],
        }
    ...
    def test_optional_nested_fields(self):
        ...
        data_without_address = {"id": 2, "name": "Bob"}
