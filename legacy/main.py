...
from politipo import pandera_to_pydantic_model, pydantic_to_pandera_schema
...
def test_schema_tools():
    ...
    simple_schema = pa.DataFrameSchema(
        {
            "name": pa.Column(
                dtype="string", checks=[pa.Check.str_length(min_value=1, max_value=50)]
            ),
    ...
    try:
        ...
        test_df = test_df.astype(
            {"name": "string", "age": "int64", "email": "string", "created_at": "datetime64[ns]"}
        )
        ...
        if hasattr(regenerated_model, "model_fields"):
            ...
            for field_name, field in regenerated_model.__fields__.items():
                ...
                try:
                    ...
                    ann = getattr(field, "annotation", "unknown")
                    print(f"    - type: {ann}")
                try:
                    print(f"    - required: {field.required}")
                except AttributeError:
                    required = not getattr(field, "allow_none", False)
                    print(f"    - required: {required}")
                if (
                    hasattr(field, "field_info")
                    and hasattr(field.field_info, "description")
        ...
        try:
            ...
            _ = simple_schema.validate(test_df)
...
def main():
    try:
        ...
        engine.register_strategy("df_to_model", DataFrameToModelStrategy())
        ...
        for record in df_dicts:
            try:
                ...
                if "addresses" in record:
                    record["addresses"] = [Address(**addr) for addr in record["addresses"]]
