...
def pydantic_to_pandera_schema(
    ...
    pandera_system: PanderaTypeSystem | None = None,
    pydantic_system: PydanticTypeSystem | None = None,
    verbose: bool = False,
) -> "pa.DataFrameSchema":
    """
    ...
    if not (isinstance(model_cls, type) and issubclass(model_cls, BaseModel)):
        raise TypeError(
            f"Input 'model_cls' must be a Pydantic BaseModel class, got {type(model_cls)}"
        )
    ...
    try:
        ...
        if canonical_model.kind != "composite":
            raise ConversionError(
                f"Pydantic model {model_cls.__name__} did not resolve to a composite canonical type."
            )

        # Convert CanonicalType back to a Pandera DataFrameSchema
        try:
            if verbose:
                print("Converting canonical type to Pandera schema")
        ...
        if not isinstance(pandera_schema, pa.DataFrameSchema):
            raise ConversionError(
                f"Pandera system did not return a DataFrameSchema instance from canonical model {canonical_model.name}. Got: {type(pandera_schema)}"
            )
        ...
        raise NotImplementedError(
            f"Conversion requires PanderaTypeSystem.from_canonical implementation for composite types: {nie}"
        ) from nie
...
def pandera_to_pydantic_model(
    ...
    if not isinstance(schema, pa.DataFrameSchema):
        raise TypeError(
            f"Input 'schema' must be a Pandera DataFrameSchema instance, got {type(schema)}"
        )
    ...
    try:
        ...
        if canonical_schema.kind != "composite":
            raise ConversionError(
                f"Pandera schema '{schema.name}' did not resolve to a composite canonical type."
            )
        ...
        # Validate the output type
        if not (
            isinstance(pydantic_model_class, type) and issubclass(pydantic_model_class, BaseModel)
        ):
            raise ConversionError(
                f"Pydantic system did not return a BaseModel subclass from canonical schema {canonical_schema.name}. Got: {type(pydantic_model_class)}"
            )
        ...
        # Specific feedback if from_canonical is the blocker
        raise NotImplementedError(
            f"Conversion requires PydanticTypeSystem.from_canonical implementation for composite types: {nie}"
        ) from nie
        ...
        raise ConversionError(
            f"Unexpected error during Pandera -> Pydantic schema conversion for schema '{schema.name}': {e}"
        ) from e
