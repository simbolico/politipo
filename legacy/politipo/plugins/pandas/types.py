...
class PandasTypeSystem(TypeSystem):
    ...
    def to_canonical(self, df_or_type: pd.DataFrame | type | np.dtype | str) -> CanonicalType:
        """
        ...
        if isinstance(df_or_type, pd.DataFrame):
            ...
            return CanonicalType(
                ...
                name="DataFrame",
                params={
                    "columns": {col: self._dtype_to_canonical(df[col].dtype) for col in df.columns}
                },
                meta=TypeMeta(
                    data={
                        "origin_system": self.name,
                        "pandas_index": {"name": df.index.name, "dtype": str(df.index.dtype)},
                    }
                ),
            )
        else:
            ...
            try:
                ...
                if isinstance(df_or_type, str):
                    ...
                    if isinstance(df_or_type, pd.StringDtype):
                        return CanonicalType(
                            ...
                            name="str",
                            meta=TypeMeta(
                                data={"origin_system": self.name, "original_dtype": str(df_or_type)}
                            ),
                        )
                    if isinstance(
                        df_or_type,
                    ...
                    raise TypeError(
                        f"Input must be DataFrame, dtype or type string, got {type(df_or_type)}"
                    )
                ...
                return CanonicalType(
                    ...
                    name="any",
                    meta=TypeMeta(
                        data={"origin_system": self.name, "original_type": str(df_or_type)}
                    ),
                )

    @lru_cache(maxsize=128)  # Added cache
    def from_canonical(self, canonical: CanonicalType) -> type[pd.DataFrame] | np.dtype | str:
        """
        ...
        if canonical.kind == "composite" and canonical.name == "DataFrame":
            ...
            try:
                ...
                raise ConversionError(
                    f"Failed to convert canonical type '{canonical}' to Pandas dtype: {e}"
                ) from e
    ...
    def detect(self, obj: Any) -> bool:
        ...
        if isinstance(obj, str) and obj in (
            "int64",
            "float64",
            "bool",
            "datetime64[ns]",
    ...
    def get_default_canonical(self) -> CanonicalType:
        ...
        return CanonicalType(
            ...
            params={
                ...
                "index": {
                    "type": CanonicalType(kind="primitive", name="int")
                },  # Default RangeIndex
            },
            meta=TypeMeta(
                data={"origin_system": self.name, "pandas_index": {"name": None, "dtype": "int64"}}
            ),
        )
    ...
    def _dtype_to_canonical(self, dtype: Any) -> CanonicalType:
        ...
        if dtype_name.startswith("int"):
            return CanonicalType(
                kind="primitive",
                name="int",
                meta=TypeMeta(data={"origin_system": self.name, "original_dtype": dtype_name}),
            )
    ...
    def _canonical_to_dtype(
        self, canonical: CanonicalType
    ) -> np.dtype | pd.api.extensions.ExtensionDtype | str:
        ...
        if canonical.kind == "primitive":
            ...
            if item_type_canonical:
                ...
                if is_extension_dtype and use_nullable_dtypes:
                    ...
                    np_item_dtype = (
                        np.object_
                    )  # Force 'object' to handle categories. Object dtype can accommodate missing values that nullable ExtensionArrays generate.
                else:
                ...
                try:
                    ...
                    return pd.CategoricalDtype(
                        categories=None, ordered=False
                    )  # Categories set later
