...
class SQLAlchemyTypeSystem(TypeSystem):
    ...
    def __init__(self):
        if sa_types is None:
            raise PolitipoError(
                "SQLAlchemy is not installed. Cannot initialize SQLAlchemyTypeSystem."
            )
    ...
    def _extract_type_info(self, type_obj: type[TypeEngine] | TypeEngine) -> dict[str, Any]:
        """
    ...
    def get_default_canonical(self) -> CanonicalType:
        ...
        return CanonicalType(
            kind="primitive", name="any", meta=TypeMeta(data={"origin_system": self.name})
        )
        ...
        if isinstance(type_obj, (TypeEngine, type)) and hasattr(type_obj, "__visit_name__"):
            ...
            if canonical_name:
                ...
                return CanonicalType(
                    ...
                    name=canonical_name,
                    params=params,
                    meta=TypeMeta(data={"origin_system": self.name, "sql_type": type_info}),
                )
            else:
                ...
                if type_name:
                    try:
                        ...
                        if type_class:
                            kwargs = {
                                k: v
                                for k, v in sql_type_info.items()
