...
try:
    ...
    Column = Table = MetaData = DeclarativeMeta = Mapped = mapped_column = registry = SchemaItem = (
        None
    )
...
class SQLAlchemyModelTypeSystem(TypeSystem):
    ...
    def __init__(self):
        if DeclarativeMeta is None:
            raise PolitipoError(
                "SQLAlchemy is not installed. Cannot initialize SQLAlchemyModelTypeSystem."
            )
        ...
        for name, column in table.columns.items():
            ...
            if column.foreign_keys:
                ...
                for fk in column.foreign_keys:
                    fk_list.append({"table": fk.column.table.name, "column": fk.column.name})
            ...
            table_args.append(
                {
                    ...
                    "kwargs": {
                        ...
                        "postgresql_where": (
                            str(getattr(index, "postgresql_where", None))
                            if getattr(index, "postgresql_where", None)
                            else None
                        ),
                    },
                }
    ...
    def from_canonical(self, canonical: CanonicalType) -> type:
        ...
        if canonical.kind != "composite":
            raise ConversionError(
                f"Cannot create SQLAlchemy model from non-composite type: {canonical.kind}"
            )

        fields_info = canonical.params.get("fields")
        if not fields_info:
            raise ConversionError(
                f"Cannot reconstruct SQLAlchemy model '{canonical.name}' without field definitions"
            )
        ...
        for name, field_info in fields_info.items():
            try:
                    ...
                    if isinstance(default_value, dict) and default_value.get("type") == "callable":
                        ...
                        if hasattr(func, default_value["name"]):
                            ...
                            column_kwargs["default"] = default_fn(
                                *default_value.get("args", []), **default_value.get("kwargs", {})
                            )
                    else:
                ...
                raise ConversionError(
                    f"Error creating column '{name}' for model '{canonical.name}': {e}"
                ) from e
        ...
        for arg in meta.get("table_args", []):
            ...
            if arg_type == "unique_constraint":
                table_args.append(UniqueConstraint(*arg["columns"], name=arg.get("name")))
            elif arg_type == "check_constraint":
                table_args.append(CheckConstraint(arg["sqltext"], name=arg.get("name")))
            elif arg_type == "index":
        ...
        try:
            ...
            raise ConversionError(
                f"Failed to create SQLAlchemy model '{canonical.name}': {e}"
            ) from e
    ...
    def detect(self, obj: Any) -> bool:
        ...
        return (
            isinstance(obj, type)
            and isinstance(getattr(obj, "__class__", None), DeclarativeMeta)
