...
class TypeMapper:
    ...
    def get_canonical_type(self, type_obj: Any, library: str) -> str | tuple:
    ...
    @lru_cache(maxsize=128)
    def get_library_type(self, canonical_type: str | tuple, library: str) -> Any:
    ...
    def map_type(self, type_obj: Any, to_library: str, from_library: str | None = None) -> Any:
    ...
    def register_library(
        ...
        if library in self.canonical_mappings and not overwrite:
            raise ValueError(
                f"Library '{library}' is already registered. Use overwrite=True to replace."
            )
    ...
    def _python_to_canonical(self, type_obj: Any) -> str | tuple:
    ...
    def _canonical_to_python(self, canonical: str | tuple) -> Any:
    ...
    def _pydantic_to_canonical(self, type_obj: Any) -> str | tuple:
    ...
    def _canonical_to_pydantic(self, canonical: str | tuple) -> Any:
            ...
            if isinstance(v, dict):
                ...
                items.append(
                    (
                        k,
                        tuple(
                            self._dict_to_tuple(item) if isinstance(item, dict) else item
                            for item in v
                        ),
                    )
                )
    ...
    def _tuple_to_dict(self, t: tuple) -> dict:
        ...
        for k, v in t:
            if isinstance(v, tuple) and all(isinstance(x, tuple) and len(x) == 2 for x in v):
                ...
                result[k] = list(
                    (
                        self._tuple_to_dict(item)
                        if isinstance(item, tuple)
                        and all(isinstance(x, tuple) and len(x) == 2 for x in item)
                        else item
                    )
                    for item in v
                )
    ...
    def detect_library(self, type_obj: Any) -> str:
        ...
        try:
            ...
            if self._get_pydantic_version() == 1:
                try:
                    ...
                    if hasattr(type_obj, "__origin__") and type_obj.__origin__ in (
                        conint,
                        confloat,
                        constr,
                    ):
        ...
        try:
            ...
            if (
                isinstance(type_obj, pd.api.extensions.ExtensionDtype)
                or isinstance(type_obj, str)
                and type_obj in ("float64", "bool", "datetime64[ns]")
            ):
                return "pandas"
        except (ImportError, AttributeError):
            pass
        ...
        try:
            ...
            if isinstance(
                type_obj, (pa.Int, pa.String, pa.Float, pa.Bool, pa.Date, pa.DateTime, pa.Decimal)
            ):
                return "pandera"
            # Check if it's a subclass of DataType
            if (
                isinstance(type_obj, type)
                and hasattr(pa, "DataType")
                and issubclass(type_obj, pa.DataType)
            ):
                return "pandera"
        except (ImportError, AttributeError):
            pass
