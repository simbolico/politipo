...
class TypeConverter:
    ...
    def _is_dataframe_type(self, type_obj: type) -> bool:
        ...
        return hasattr(type_obj, "__module__") and type_obj.__module__ in [
            "pandas.core.frame",
    ...
    def convert(self, data: Any) -> Any:
        ...
        try:
            ...
            if isinstance(self.to_type, pa.DataFrameSchema):
                ...
                try:
                    ...
                    if "pandas" in str(e):
                        raise ValueError(
                            "Pandas is required for Polars to Pandera validation."
                        ) from e
                    raise

                raise ValueError(
                    "Data must be a Pandas or Polars DataFrame for Pandera schema validation"
                )
        except ImportError:
            if hasattr(self.to_type, "validate") and hasattr(
                self.to_type, "columns"
            ):  # Rough check for Pandera schema
        ...
        if "pydantic" not in sys.modules or sys.modules["pydantic"] is None:
            if (
                (hasattr(self.from_type, "model_dump") or hasattr(self.to_type, "model_validate"))
                or ("pydantic" in str(self.from_type) and "SQLModel" not in str(self.from_type))
                or ("pydantic" in str(self.to_type) and "SQLModel" not in str(self.to_type))
            ):
        ...
        try:
            ...
            if issubclass(self.to_type, BaseModel):
                if self.from_type is dict:
                    ...
                    data_dict = {
                        c.name: getattr(data, c.name) for c in self.from_type.__table__.columns
                    }
                    return self.to_type.model_validate(data_dict)
                    ...
                    if self.from_type.__module__ == "pandas.core.frame":
                        ...
                        return [self.to_type.model_validate(d) for d in list_of_dicts]
                raise ValueError(
                    f"Unsupported from_type '{self.from_type}' for to_type=Pydantic/SQLModel"
                )
        except ImportError:
        ...
        if hasattr(self.to_type, "__table__"):
            ...
            raise ValueError(
                f"Unsupported from_type '{self.from_type}' for to_type=SQLAlchemy model"
            )
            ...
            if self.to_type is pd.DataFrame:
                if isinstance(data, list):
                    ...
                    try:
                        ...
                        raise ImportError(
                            "Pydantic is required for converting Pydantic models to pd.DataFrame."
                        )
        ...
        if (
            hasattr(self.from_type, "__module__")
            and self.from_type.__module__ == "pandas.core.frame"
        ):
            # Special check for test_collection_to_pydantic_from_pandas
            if sys.modules.get("pydantic") is None:
                raise ImportError(
                    "Pydantic is required for converting pd.DataFrame to Pydantic models."
                )
        ...
        if (
            hasattr(self.to_type, "__module__")
            and self.to_type.__module__ == "polars.dataframe.frame"
        ):
            import polars as pl
            ...
            if self.to_type is pl.DataFrame:
                if isinstance(data, list):
                    ...
                    try:
                        ...
                        raise ImportError(
                            "Pydantic is required for converting Pydantic models to pl.DataFrame."
                        )
        ...
        if (
            hasattr(self.from_type, "__module__")
            and self.from_type.__module__ == "polars.dataframe.frame"
        ):
            if sys.modules.get("pydantic") is None:
                raise ImportError(
                    "Pydantic is required for converting pl.DataFrame to Pydantic models."
                )
    ...
    def convert_collection(self, data: list[Any], coerce: bool = False) -> Any:
        ...
        if (
            ...
            and data.__class__.__module__ in ["pandas.core.frame", "polars.dataframe.frame"]
        ):
            return self.convert(data)
        ...
        try:
            ...
            for field_name, field_type in annotations.items():
                if field_name in processed_data:
                    ...
                    if self._is_pydantic_or_sqlmodel(field_type) and isinstance(
                        processed_data[field_name], dict
                    ):
                        # Create nested model instance
                        processed_data[field_name] = self._convert_nested_field(
                            processed_data[field_name], field_type
                        )
                    ...
                    # Handle lists of Pydantic models
                        ...
                        and self._is_pydantic_or_sqlmodel(get_args(field_type)[0])
                        and isinstance(processed_data[field_name], list)
                    ):
            ...
            for field_name, field_info in fields.items():
                ...
                if isinstance(pa_type, pa.String):
                    if "min_length" in constraints and "max_length" in constraints:
                        ...
                        checks.append(pa.Check.str_length(min_value=constraints["min_length"]))
                    elif "max_length" in constraints:
                        checks.append(pa.Check.str_length(max_value=constraints["max_length"]))
            ...
            for col_name, col in schema.columns.items():
                ...
                if col.checks:
                    for check in col.checks:
                        ...
                        check_name = (
                            check_fn.__name__ if hasattr(check_fn, "__name__") else str(check_fn)
                        )
                ...
                for col_name, col_data in df.items():
                    if col_name in fields:
                        ...
                        if "gt" in constraints and (col_data <= constraints["gt"]).any():
                            ...
                            raise pa.errors.SchemaError(schema=None, data=df, message=error_message)
                        ...
                        if "min_length" in constraints and isinstance(col_data.iloc[0], str):
                            ...
                            if (lengths < constraints["min_length"]).any():
                                ...
                                raise pa.errors.SchemaError(
                                    schema=None, data=df, message=error_message
                                )
                ...
                if hasattr(field_info, "metadata") and field_info.metadata:
                    for item in field_info.metadata:
                        ...
                        if hasattr(item, "__class__"):
                            ...
                            elif hasattr(item, "pattern") and class_name in [
                                "Pattern",
                                "RegexPattern",
                            ]:
                            ...
                            # Handle _PydanticGeneralMetadata pattern
                                ...
                                hasattr(item, "pattern")
                                and class_name == "_PydanticGeneralMetadata"
                            ):
                                constraints["regex"] = item.pattern
            ...
            # Support for Pydantic v1
                ...
                if hasattr(field_info, "type_"):
                    if (
                        ...
                        and field_info.type_.__origin__ is typing.Annotated
                    ):
