...
class ConversionEngine:
    ...
    def __init__(self, strategies: list[ConversionStrategy] | None = None):
        self._strategies = strategies or []
        self._type_systems: dict[str, TypeSystem] = {}

    def register_strategy(
        self, strategy: str | ConversionStrategy, strategy_impl: ConversionStrategy | None = None
    ) -> None:
        """
    ...
    def convert(
        self, value: Any, target: str | type | CanonicalType | TypeSystem, strict: bool = True
    ) -> Any:
        """
        ...
        try:
            ...
            source_system = self._detect_system_for_type(type(value)) or self._get_type_system(
                "python"
            )
            target_system = (
                self._detect_system_for_type(target)
                if isinstance(target, type)
                else (
                    self._get_type_system(target)
                    if isinstance(target, str)
                    else target if isinstance(target, TypeSystem) else None
                )
            )
            ...
            raise ConversionError(
                f"Conversion failed using strategy {type(strategy).__name__}: {e}"
            ) from e

    def _resolve_canonical(
        self,
        value_or_type: Any,  # Can be the value (for source) or the type (for target)
        type_spec: str | type | CanonicalType | TypeSystem | None,
        is_source: bool,
    ) -> CanonicalType:
        """
    ...
    def _detect_system_for_type(self, native_type: type) -> TypeSystem | None:
    ...
    def _get_type_system(self, type_spec: CanonicalType | TypeSystem | str) -> TypeSystem | None:
    ...
    def _find_strategy(
        self, source: CanonicalType, target: CanonicalType
