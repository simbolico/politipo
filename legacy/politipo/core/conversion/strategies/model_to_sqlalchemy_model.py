...
class ModelToSQLAlchemyModelStrategy(ConversionStrategy):
    ...
    def can_handle(self, source: CanonicalType, target: CanonicalType) -> bool:
        ...
        return (
            ...
            and source.meta.get("origin_system") in ("pydantic", "sqlmodel")
    ...
    def convert(self, value: Any, context: ConversionContext) -> Any:
        ...
        if not _PYDANTIC_AVAILABLE:
            ...
            is_sqlmodel_source = (
                context.source.meta and context.source.meta.get("origin_system") == "sqlmodel"
            )
            if is_sqlmodel_source:
                raise PolitipoError(
                    "Pydantic (dependency of SQLModel) is required for ModelToSQLAlchemyModelStrategy"
                )
        ...
        try:
            if hasattr(value, "model_dump"):  # Pydantic v2 preferred
                ...
                raise ConversionError(
                    f"Source object {type(value)} does not have 'model_dump' or 'dict' method."
                )
        except Exception as e:
            raise ConversionError(
                f"Failed to convert source model {type(value).__name__} to dict: {e}"
            ) from e
        ...
        try:
            ...
            if not context.strict:
                ...
                data_to_instantiate = {
                    k: v for k, v in data_dict.items() if k in valid_column_names
                }
            ...
            if context.strict:
                raise ConversionError(
                    f"Failed to instantiate {model_class.__name__} from dict {data_dict}. Strict mode error: {e}"
                ) from e
            else:
            ...
            raise ConversionError(
                f"Failed to create SQLAlchemy model instance {model_class.__name__} from dict: {e}"
            ) from e
