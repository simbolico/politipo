...
class DataFrameToModelStrategy(ConversionStrategy):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    results.append(self._convert_row(self._make_row_hashable(data), model_type))
                except ValidationError as e:
                    if context.strict:
                        raise ConversionError(f"Row {idx} validation failed: {str(e)}") from e
            ...
            raise ConversionError(f"Failed to convert DataFrame to {model_type.__name__}") from e

    @lru_cache(maxsize=1024)
    def _convert_row(
        self, row_hash: tuple[tuple[str, Any], ...], model_type: type[BaseModel]
    ) -> BaseModel:
        """
    ...
    def _make_row_hashable(self, row_dict: dict[str, Any]) -> tuple[tuple[str, Any], ...]:
        ...
        return tuple(
            sorted(
                (k, v) if isinstance(v, (str, int, float, bool, type(None))) else (k, str(v))
