...
class PandasToModelListStrategy(ConversionStrategy):
    ...
    def can_handle(self, source: CanonicalType, target: CanonicalType) -> bool:
        ...
        return (
            ...
            and source.name == "DataFrame"
            and target.kind == "container"
            and target.name == "list"
            and target.params.get("item_type", {}).kind == "composite"
        )
    ...
    def convert(self, value: Any, context: ConversionContext) -> list[Any]:
        ...
        if not item_system:
            raise ConversionError(
                "Target type system not found in context for PandasToModelListStrategy"
            )
        ...
        try:
            ...
            if not isinstance(model_class, type):
                raise ConversionError(
                    f"Target system did not return a valid class for list item {item_canonical_type.name}"
                )
        except NotImplementedError:
            raise ConversionError(
                f"Target system '{item_system.name}' cannot reconstruct model from canonical type."
            )
        ...
        try:
            ...
            raise ConversionError(
                f"Failed to create model instances of {model_class.__name__}: {e}"
            )
    ...
    def _create_model_instance(self, model_class: type, data: dict[str, Any], strict: bool) -> Any:
        ...
        try:
            ...
            if strict:
                ...
                raise ConversionError(
                    f"Failed to create {model_class.__name__} instance: {e}"
                ) from e
            else:
                # If even filtering + construction failed in non-strict, raise with context
                raise ConversionError(
                    f"Non-strict creation failed for {model_class.__name__} after filtering: {e}"
                ) from e
