    ...
    def validate(self, val: Any) -> bool:
        try:
            ...
            if numeric_val < self.value:
                raise ConstraintViolationError(f"Value {val} is less than minimum {self.value}")
            return True
...
class MaxValue(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            numeric_val = Decimal(str(val)) if not isinstance(val, (int, float, Decimal)) else val
            if numeric_val > self.value:
                raise ConstraintViolationError(f"Value {val} is greater than maximum {self.value}")
            return True
        except (TypeError, ValueError, InvalidOperation) as e:
...
class MaxLength(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            if len(val) > self.value:
                raise ConstraintViolationError(f"Length {len(val)} exceeds maximum {self.value}")
            return True
        except TypeError as e:
            raise ConstraintViolationError(
...
class GreaterThan(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            numeric_val = Decimal(str(val)) if not isinstance(val, (int, float, Decimal)) else val
            if numeric_val <= self.value:
                raise ConstraintViolationError(f"Value {val} is not greater than {self.value}")
            return True
        except (TypeError, ValueError, InvalidOperation) as e:
...
class LessThan(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            numeric_val = Decimal(str(val)) if not isinstance(val, (int, float, Decimal)) else val
            if numeric_val >= self.value:
                raise ConstraintViolationError(f"Value {val} is not less than {self.value}")
            return True
        except (TypeError, ValueError, InvalidOperation) as e:
...
class GreaterThanOrEqual(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            numeric_val = Decimal(str(val)) if not isinstance(val, (int, float, Decimal)) else val
            if numeric_val < self.value:
                raise ConstraintViolationError(f"Value {val} is less than {self.value}")
            return True
        except (TypeError, ValueError, InvalidOperation) as e:
            raise ConstraintViolationError(
                f"Invalid numeric value for GreaterThanOrEqual: {val}"
            ) from e
...
class LessThanOrEqual(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            numeric_val = Decimal(str(val)) if not isinstance(val, (int, float, Decimal)) else val
            if numeric_val > self.value:
                raise ConstraintViolationError(f"Value {val} is greater than {self.value}")
            return True
        except (TypeError, ValueError, InvalidOperation) as e:
            raise ConstraintViolationError(
                f"Invalid numeric value for LessThanOrEqual: {val}"
            ) from e
...
class Pattern(Constraint):
    ...
    def __post_init__(self):
        ...
        try:
            object.__setattr__(
                self,
                "_compiled_regex",
                re.compile(self.regex) if isinstance(self.regex, str) else self.regex,
            )
    ...
    def validate(self, val: Any) -> bool:
        if not isinstance(val, str):
            raise ConstraintViolationError(
                f"Pattern constraint requires string value, got {type(val)}"
            )
        if not self._compiled_regex.match(val):
            raise ConstraintViolationError(f"Value '{val}' does not match pattern '{self.regex}'")
        return True

    def to_dict(self) -> dict[str, Any]:
        return {
            "type": "pattern",
            "regex": self.regex if isinstance(self.regex, str) else self.regex.pattern,
        }
...
class MultipleOf(Constraint):
    ...
    def validate(self, val: Any) -> bool:
        try:
            numeric_val = (
                Decimal(str(val)) if not isinstance(val, (int, float, Decimal)) else Decimal(val)
            )
            multiple = Decimal(str(self.value))
            if (numeric_val % multiple) != 0:
                raise ConstraintViolationError(f"Value {val} is not a multiple of {self.value}")
            return True
        except (TypeError, ValueError, InvalidOperation) as e:
...
class OneOf(Constraint):
    ...
    def __post_init__(self):
        if not isinstance(self.allowed_values, (list, tuple, set)):
            raise TypeError(
                f"OneOf requires a sequence of allowed values, got {type(self.allowed_values)}"
            )
